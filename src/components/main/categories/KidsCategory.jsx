...
function KidsCategory({ openDropdown, setOpenDropdown, addAd_data }) {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post(
        `https://pak-deals-backend.vercel.app/api/ads/add-kids-ad/${userId}`,
        form,
      )
      .then((res) => {
        toast.success(res?.data?.message || "Kids ad posted successfully!");
        localStorage.setItem("table_name", "kids_ads");
  ...
  const renderDropdown = (label, key, dataKey, scrollable = false) => (
    <div className="w-full relative">
      ...
      {openDropdown === key && (
        ...
        >
          {(addAd_data.kids.find((i) => i[dataKey])?.[dataKey] || []).map(
            (item) => (
              <div
                key={item.id}
    ...
    <form className="space-y-8 bg-white" onSubmit={handleSubmit}>
      ...
      <div className="bg-blue-50/30 p-6 rounded-2xl border border-blue-100 space-y-6">
        <h3 className="flex items-center gap-2 text-blue-900 font-black uppercase tracking-wider text-sm">
          <Baby size={18} /> Basic Information
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {renderDropdown("Sub Category", "subCategory", "kidsSubCategories")}
          {renderInput(
            "Ad Title",
            "adTitle",
            "text",
            formData.adTitle,
            "e.g. Fisher-Price Baby Jumperoo",
          )}
        </div>
        <textarea
          rows={4}
          name="description"
          placeholder="Mention features, safety certifications, or usage history..."
      ...
      <div className="space-y-6">
        <h3 className="flex items-center gap-2 text-blue-900 font-black uppercase tracking-wider text-sm">
          <ToyBrick size={18} /> Product Specs
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {renderDropdown("Item Type", "itemType", "kidsItemType", true)}
          {renderDropdown("Age Group", "ageGroup", "kidsAgeGroup", true)}
          {renderInput(
            "Brand",
            "brand",
            "text",
            formData.brand,
            "e.g. Mothercare, LEGO, Barbie",
          )}
      ...
      >
        {loading ? "Posting Ad..." : "Submit Kids Ad"}
      </button>
    </form>
  );
}
