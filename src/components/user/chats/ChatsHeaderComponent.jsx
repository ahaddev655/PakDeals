...
function ChatsHeaderComponent() {
  ...
  return (
    <div className="bg-white flex items-center justify-between md:px-12 px-6 py-6 border border-t-0 border-l-0 border-gray-200">
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          {currentUser.status === "1" ? (
            <span className="font-medium text-green-500 text-sm">Online</span>
          ) : (
            <span className="font-medium text-gray-500 text-sm">Offline</span>
          )}
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <div
          className={`absolute top-13 -left-25 w-40 z-10 bg-white shadow-lg border border-gray-200 p-1 rounded-md transition-all duration-300 origin-top transform ${
            dropdownToggle ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"
          }`}
        >
